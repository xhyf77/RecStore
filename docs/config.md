...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `capacity` | integer | 是 | Key 容量上限 |
| `index` | object | 是 | 包含 `type`，SSD 索引需提供 `index.path` |
| `value` | object | 是 | 包含 `type`，单层存储使用 `value.path`，TIERED 使用 allocator path |
...
- `SSD_VALUE_STORE`：需要非空 `value.path` 和 `value.ssd_allocator`
- `TIERED_VALUE_STORE`：需要 `value.dram_allocator` 和 `value.ssd_allocator`，其中 `value.ssd_allocator.path` 必须非空

#### 引擎类型自动推导规则

`base::ResolveEngine` 根据 `index.type` 和 `value.type` 组合解析引擎：

- `index.type` 取值：`DRAM_EXTENDIBLE_HASH` / `DRAM_UNORDERED_MAP` / `DRAM_PET_HASH`
- `value.type` 取值：`DRAM_VALUE_STORE` / `SSD_VALUE_STORE` / `TIERED_VALUE_STORE`
- 旧字段 `path`、`index_type`、`value_type`、`shmcapacity`、`ssdcapacity`、`index.io.file_path`、`value.ssd_allocator.io.file_path` 会被拒绝
