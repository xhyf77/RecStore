...
public:
   ...
   */
  virtual bool
  update(const char* key, size_t key_sz, const char* value, size_t value_sz) {
    dict_->Set(*(uint64_t*)key, *(uint64_t*)value);
    return true;
  }
...
extern "C" hash_api*
create_hashtable(const hashtable_options_t& opt, unsigned sz, unsigned tnum) {
  ;
