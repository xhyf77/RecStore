...
local_lookup_flat_torch(const torch::Tensor& keys, int64_t embedding_dim) {
  ...
  if (gpu::CanUseGpuCache(keys, embedding_dim)) {
    try {
      ...
      auto miss_values_cuda = miss_values.to(orig_device, /*non_blocking=*/false);
      ...
      LOG(WARNING) << "GPU cache lookup failed; clearing cache and falling back: "
                   << e.what();
...
void local_update_flat_torch(const std::string& table_name,
  ...
  if (gpu::CanUseGpuCache(keys, grads.size(1)) && keys.is_cuda() &&
    ...
    try {
      ...
      auto refreshed_cpu = BackendLocalLookupFlat(kv_op,
                                                  cpu_keys.contiguous(),
                                                  keys.device(),
                                                  /*result_on_cuda=*/false,
                                                  grads.size(1),
