...
public:
  ...
  PetDatasetReader(int reader_thread_count,
                   uint64_t hashed_key_space_M,
                   const std::string& dataset_file_wildcard_str,
                   bool repeat)
      : reader_thread_count_(reader_thread_count),
        hashed_key_space_(hashed_key_space_M * 1024 * 1024LL),
        repeat_(repeat) {
    int count = 0;
